...
void Play()
{
		...
		if (ui->GetIsChoise())	//選択していたら
		{
			...
			else if (ui->GetChoiseMenu() == (int)MENU_SOUBI)	//装備を選んだ時
			{
				...
				if (Equip_select->GetSelectFlg())				//武器か、防具か選択されたら
				{
					...
					Equip_select->SetIsDrawImage(false);		//武器、防具の選択肢のUI非表示

					if (!Yes_No->GetIsKeyOpe())	//装備するものを選んでいないとき
					{
	...
	else			//メニュー描画終了してたら
	{
		...
		Equip_select->SetSelectFlg(false);			//武器、防具選択していない
		Equip_select->SetIsKeyOpe(true);			//武器、防具キー操作可能
		Equip_select->NowSelectReset();				//武器、防具の選択状態リセット
		Equip_select->SetIsDrawImage(true);			//武器、防具のUI表示
