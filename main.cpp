...
int WINAPI WinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, LPSTR lpCmdLine, int nCmdShow)
{
	...
	ui = new UI();		//UI作成
	if (ui->AddUiImage(UI_DIR, UI_START_NAME,(int)UI_START) == false) { return -1; }	//start画像追加
	if (ui->AddUiImage(UI_DIR, UI_END_NAME,(int)UI_END) == false) { return -1; }	//end画像追加
...
void Title_Draw()
{
	...
	text->Draw(GAME_WIDTH / 2 - text->GetWidth() / 2, DEFAULT_TEXT_Y, str.size(), false);	//描画（矢印付き）

	ui->DrawUiImage(GAME_WIDTH / 2 - ui->GetUiImageWidth((int)UI_START) / 2, 450, (int)UI_START);	//start画像の描画

	ui->DrawUiImage(GAME_WIDTH / 2 - ui->GetUiImageWidth((int)UI_START) / 2, 500, (int)UI_END);		//end画像の描画

	ui->DrawUiImage(GAME_WIDTH / 2 - ui->GetUiImageWidth((int)UI_START) / 2, DEFAULT_TEXT_Y,(int)UI_ARROW);	//選択用のUI描画
